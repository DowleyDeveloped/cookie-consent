...
Craft.CookieConsent.CookieIndex = Craft.BaseElementIndex.extend({
	...
	_createTemplate: async function () {
		...
		Craft.sendActionRequest("POST", "dowley-cookieconsent/cookies/edit").then(
			({ data: table }) => {
	...
	_scanForCookies: async function () {
		...
		Craft.sendActionRequest('GET', 'dowley-cookieconsent/cookies/scan').then((response) =>
		{
			if (response.success) {
		...
		//
		// $.ajax({
		//   url: '/actions/dowley-cookieconsent/cookies/scan',
		//   method: 'GET',
		//   success(response) {
		// 	if (response.success) {
		// 	  console.log('✅ Puppeteer script ran successfully:', response.output);
