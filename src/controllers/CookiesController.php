...
class CookiesController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $variables["elements"] = CookieElement::find()->all();

        return $this->renderTemplate(
            "dowley-cookieconsent/cookies/index",
            $variables
        );
    }
    ...
    public function actionEdit()
    {
        ...
        if (!$success) {
            return $this->asModelFailure(
                $template,
                Craft::t("app", "Couldn’t create {type}.", [
        ...
        $response = $this->asModelSuccess(
            $template,
            Craft::t("dowley-cookieconsent", "{type} created.", [
        ...
        if (!$this->request->getAcceptsJson()) {
            $response->redirect(
                UrlHelper::urlWithParams($editUrl, [
                    "fresh" => 1,
                ])
            );
        }
    ...
    public function actionGetConsent()
    {
        ...
        if ($row) {
            $options = [$row["accepted"], $row["rejected"]];
        }
